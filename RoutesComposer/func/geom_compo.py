...
class GeomCompo:
    ...
    def create_merged_geometry(self, segment_ids: List[int]) -> Tuple[QgsGeometry, List[int]]:
        """
            ...
            if len(segment_ids) > 1:
                ...
                if i < len(segment_ids) - 1:
                    ...
                    if next_points:
                        ...
                        if self.points_are_equal(current_points[0], next_points[0]) or \
                        self.points_are_equal(current_points[0], next_points[-1]):
                        ...
                        elif self.points_are_equal(current_points[-1], next_points[0]) or \
                            self.points_are_equal(current_points[-1], next_points[-1]):
                    ...
                    if prev_points:
                        ...
                        if self.points_are_equal(prev_points[0], current_points[-1]) or \
                        self.points_are_equal(prev_points[-1], current_points[-1]):
                            result_points.extend(reversed(current_points))
                        # Le premier point touche le segment précédent.
                        elif self.points_are_equal(prev_points[0], current_points[0]) or \
                        self.points_are_equal(prev_points[-1], current_points[0]):
    ...
    def create_compositions_geometries(self, progress_bar):
        ...
        for composition in get_features_list(self.compositions_layer):
            ...
            if isinstance(segments_str, str):
                ...
                new_geometry, non_connected = self.create_merged_geometry(segment_ids)
