...
def highlight_errors(errors, segments_layer, id_column_name):
    ...
    for error in errors:
        if error['error_type'] == 'discontinuity':
            ...
            segment_id1_feature = next(segments_layer.getFeatures(QgsFeatureRequest().setFilterExpression(f'"{id_column_name}" = \'{segment_id1}\'')), None)
            geom1 = segment_id1_feature.geometry() if segment_id1_feature else None

            segment_id2_feature = next(segments_layer.getFeatures(QgsFeatureRequest().setFilterExpression(f'"{id_column_name}" = \'{segment_id2}\'')), None)
