...
def verify_segments(segments_layer, compositions_layer, segments_column_name):
    """
    ...
    segments_geom_dict = {str(seg['id']): seg.geometry() for seg in utils.get_features_list(segments_layer)}
    ...
    for composition in compositions:
        ...
        for i, current_segment_id in enumerate(segments_list):
            ...
            if i < len(segments_list) - 1 :
                ...
                current_geom = cast(QgsGeometry, segments_geom_dict[current_segment_id])
                next_geom = cast(QgsGeometry, segments_geom_dict[next_segment_id])
                ...
                if current_points and next_points:
                    ...
                    if distance_1 < 0.1 or distance_2 < 0.1 or distance_3 < 0.1 or distance_4 < 0.1:
                        pass
                    else:
...
def highlight_errors(errors, segments_layer):
    """
    ...
    error_layer = next((layer for layer in existing_layers if layer.name() == error_layer_name), None)
    if error_layer is None:
        error_layer = QgsVectorLayer("LineString?crs=" + segments_layer.crs().authid(), "Erreurs", "memory")
    ...
    for error in errors:
        if error['error_type'] == 'discontinuity':
            ...
            segment_id1_feature = next(segments_layer.getFeatures(QgsFeatureRequest().setFilterExpression(f'"id" = \'{segment_id1}\'')), None)
            geom1 = segment_id1_feature.geometry() if segment_id1_feature else None

            segment_id2_feature = next(segments_layer.getFeatures(QgsFeatureRequest().setFilterExpression(f'"id" = \'{segment_id2}\'')), None)
            geom2 = segment_id2_feature.geometry() if segment_id2_feature else None

            if geom1 and geom2 and geom1.isGeosValid() and geom2.isGeosValid():
                feat1 = QgsFeature()
                feat1.setGeometry(geom1)
                feat1.setAttributes([segment_id1, error['error_type'], error['composition_id']])
                features.append(feat1)

                feat2 = QgsFeature()
                feat2.setGeometry(geom2)
                feat2.setAttributes([segment_id2, error['error_type'], error['composition_id']])
