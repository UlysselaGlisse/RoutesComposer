...
class AdvancedOptions(QObject):
    ...
    def on_segments_attr_selected(self):
        if self.dialog.ui.segments_attr_combo.currentText():
            selected_segments_attr = self.dialog.ui.segments_attr_combo.currentText()
            settings = QSettings()
            ...
            field_index = self.dialog.layer_manager.selected_segments_layer.fields().indexOf(selected_segments_attr)
            if field_index != -1:
    ...
    def on_compositions_attr_selected(self):
        if self.dialog.ui.compositions_attr_combo.currentText():
            selected_compositions_attr = self.dialog.ui.compositions_attr_combo.currentText()
            settings = QSettings()
    ...
    def update_attr_combos(self):
        if not self.dialog.layer_manager.selected_segments_layer or not self.dialog.layer_manager.selected_compositions_layer:
            return

        self.dialog.ui.segments_attr_combo.clear()
        self.dialog.ui.compositions_attr_combo.clear()

        for field in self.dialog.layer_manager.selected_segments_layer.fields():
            self.dialog.ui.segments_attr_combo.addItem(field.name())

        for field in self.dialog.layer_manager.selected_compositions_layer.fields():
        ...
        if saved_segments_attr:
            segments_attr_index = self.dialog.ui.segments_attr_combo.findText(saved_segments_attr)
            if segments_attr_index >= 0:
                self.dialog.ui.segments_attr_combo.setCurrentIndex(segments_attr_index)

        if saved_compositions_attr:
            compositions_attr_index = self.dialog.ui.compositions_attr_combo.findText(saved_compositions_attr)
            if compositions_attr_index >= 0:
                self.dialog.ui.compositions_attr_combo.setCurrentIndex(compositions_attr_index)
