...
class RoutesComposerDialog(QDialog):
    ...
    def on_layer_selected(self):
        ...
        if project:
            ...
            if self.selected_segments_layer:
                ...
                # Si la couche segment n'a pas de géométrie on renvoie une erreur.'
            ...
            if self.selected_compositions_layer:
                ...
                if isinstance(self.selected_compositions_layer, QgsVectorLayer) and self.selected_compositions_layer.isSpatial():
                    ...
                    self.create_or_update_geom_button.clicked.disconnect()  # Déconnecter l'ancien signal
                    self.create_or_update_geom_button.clicked.connect(self.update_geometries)  # Connecter au nouveau signal
                else:
                    self.geom_checkbox.setVisible(False)
    ...
    def check_errors(self):
        ...
        errors = verify_segments(segments_layer, compositions_layer, segments_column_name)

        if errors:
            # highlight_errors(errors, segments_layer)
            self.close()
            error_dialog = ErrorDialog(errors, segments_layer, self)
            error_dialog.show()
        else:
    ...
    def create_geometries(self):
        ...
        if errors_messages:
            ...
            error_dialog = ErrorDialog(errors_messages, segments_layer, self)
            error_dialog.show()
